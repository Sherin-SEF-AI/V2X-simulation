#!/usr/bin/env python3
# Enhanced V2X Simulation Application with Advanced Visualizations
...
class MessageGraphicsItem(QGraphicsItem):
    ...
    def __init__(self, sender_pos, receivers, message_type):
        ...
        self.animation_speed = 1.0 / self.time_to_live

        # Set position
        self.setPos(sender_pos)

        # Set z-value to be above vehicles
        self.setZValue(20)

    def boundingRect(self):
        """Define the bounding rectangle for the message"""
    ...
    def paint(self, painter, option, widget):
        ...
        # Adjust alpha based on time to live
        alpha = int(150 * (self.time_to_live / Config.MESSAGE_TTL))
...
class GridGraphicsItem(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        while x <= self.rect.right():
            # Use QLineF instead of direct coordinates
...
class StatisticsPanel(QWidget):
    ...
    def initUI(self):
        ...
        perf_group = QGroupBox("Performance Indicators")
        perf_layout = QFormLayout()

        self.traffic_efficiency_label = QLabel("100%")
        perf_layout.addRow("Traffic Efficiency:", self.traffic_efficiency_label)

        self.comm_reliability_label = QLabel("100%")
        perf_layout.addRow("Comm. Reliability:", self.comm_reliability_label)

        self.emergency_response_label = QLabel("N/A")
        perf_layout.addRow("Emergency Response:", self.emergency_response_label)

        self.network_load_label = QLabel("0%")
        perf_layout.addRow("Network Load:", self.network_load_label)

        self.success_rate_label = QLabel("100%")
        perf_layout.addRow("Msg. Success Rate:", self.success_rate_label)
        ...
        msg_group = QGroupBox("Message Distribution")
        msg_layout = QVBoxLayout()

        # Pie chart for message types
        self.msg_chart = QChart()
        self.msg_chart.setTitle("Message Types")
        self.msg_chart.setAnimationOptions(QChart.AnimationOption.SeriesAnimations)
        self.msg_chart.legend().setVisible(True)
        self.msg_chart.legend().setAlignment(Qt.AlignmentFlag.AlignBottom)

        self.msg_series = QPieSeries()
        # Initial empty series
        self.msg_series.append("No Data", 1)
        self.msg_chart.addSeries(self.msg_series)

        chart_view = QChartView(self.msg_chart)
        chart_view.setRenderHint(QPainter.RenderHint.Antialiasing)

        msg_layout.addWidget(chart_view)
        msg_group.setLayout(msg_layout)
        comm_layout.addWidget(msg_group)
        ...
        # Traffic flow chart
        traffic_chart_group = QGroupBox("Traffic Flow")
        traffic_chart_layout = QVBoxLayout()

        self.traffic_chart = QChart()
        self.traffic_chart.setTitle("Vehicle Count Over Time")
        self.traffic_chart.setAnimationOptions(QChart.AnimationOption.NoAnimation)

        self.traffic_series = QLineSeries()
        self.traffic_chart.addSeries(self.traffic_series)

        # Add axes
        self.traffic_axis_x = QValueAxis()
        self.traffic_axis_x.setTitleText("Time (s)")
        self.traffic_axis_y = QValueAxis()
        self.traffic_axis_y.setTitleText("Vehicle Count")

        self.traffic_chart.addAxis(self.traffic_axis_x, Qt.AlignmentFlag.AlignBottom)
        self.traffic_chart.addAxis(self.traffic_axis_y, Qt.AlignmentFlag.AlignLeft)
        self.traffic_series.attachAxis(self.traffic_axis_x)
        self.traffic_series.attachAxis(self.traffic_axis_y)

        traffic_chart_view = QChartView(self.traffic_chart)
        traffic_chart_view.setRenderHint(QPainter.RenderHint.Antialiasing)

        traffic_chart_layout.addWidget(traffic_chart_view)
        traffic_chart_group.setLayout(traffic_chart_layout)
        traffic_layout.addWidget(traffic_chart_group)

        # Vehicle type distribution
        vehicle_dist_group = QGroupBox("Vehicle Distribution")
        vehicle_dist_layout = QVBoxLayout()

        self.vehicle_chart = QChart()
        self.vehicle_chart.setTitle("Vehicle Types")
        self.vehicle_chart.setAnimationOptions(QChart.AnimationOption.SeriesAnimations)

        self.vehicle_series = QBarSeries()
        self.vehicle_barset = QBarSet("Count")

        # Create categories for the x-axis
        categories = [vtype.value for vtype in VehicleType]

        # Add axis
        self.vehicle_axis_x = QBarCategoryAxis()
        self.vehicle_axis_x.append(categories)
        self.vehicle_axis_y = QValueAxis()
        self.vehicle_axis_y.setRange(0, 10)  # Initial range

        self.vehicle_chart.addAxis(self.vehicle_axis_x, Qt.AlignmentFlag.AlignBottom)
        self.vehicle_chart.addAxis(self.vehicle_axis_y, Qt.AlignmentFlag.AlignLeft)

        # Initial empty data
        for _ in range(len(categories)):
            self.vehicle_barset.append(0)

        self.vehicle_series.append(self.vehicle_barset)
        self.vehicle_chart.addSeries(self.vehicle_series)
        self.vehicle_series.attachAxis(self.vehicle_axis_x)
        self.vehicle_series.attachAxis(self.vehicle_axis_y)
        ...
        vehicle_chart_view = QChartView(self.vehicle_chart)
        vehicle_chart_view.setRenderHint(QPainter.RenderHint.Antialiasing)

        vehicle_dist_layout.addWidget(vehicle_chart_view)
        vehicle_dist_group.setLayout(vehicle_dist_layout)
        traffic_layout.addWidget(vehicle_dist_group)
    ...
    def update_statistics(self, stats):
        ...
        if stats.get("emergency_response_time", 0) > 0:
            self.emergency_response_label.setText(
                f"{stats['emergency_response_time']:.1f}s"
            )
        else:
            self.emergency_response_label.setText("N/A")
        ...
        if "message_types" in stats and stats["message_types"]:
            self.update_message_chart(stats["message_types"])
        ...
        if (
            "history" in stats
            and "time_points" in stats["history"]
            and stats["history"]["time_points"]
        ):
            self.update_traffic_chart(stats["history"])

        # Update vehicle distribution chart
        if "vehicles_by_type" in stats and stats["vehicles_by_type"]:
            self.update_vehicle_chart(stats["vehicles_by_type"])

    def update_message_chart(self, message_types):
        """Update message distribution chart"""
        self.msg_series.clear()
        ...
        for msg_type, count in message_types.items():
            percentage = (count / total) * 100
            if percentage > 1.0:  # Only show types with >1% share
                self.msg_series.append(msg_type, count)

    def update_traffic_chart(self, history):
        """Update traffic flow chart"""
        ...
        for i in range(len(time_points)):
            self.traffic_series.append(time_points[i], vehicle_counts[i])

        # Update axes ranges
        min_time = min(time_points) if time_points else 0
        max_time = max(time_points) if time_points else 100
        max_count = max(vehicle_counts) if vehicle_counts else 10

        self.traffic_axis_x.setRange(min_time, max(min_time + 60, max_time))
        self.traffic_axis_y.setRange(0, max(10, max_count * 1.1))

    def update_vehicle_chart(self, vehicles_by_type):
        """Update vehicle distribution chart"""
        # Remove old bar set from series
        self.vehicle_series.clear()

        # Create a new bar set
        self.vehicle_barset = QBarSet("Count")

        # Add data for each vehicle type
        for vtype in VehicleType:
            count = vehicles_by_type.get(vtype.value, 0)
            self.vehicle_barset.append(count)

        # Add the updated bar set to the series
        self.vehicle_series.append(self.vehicle_barset)

        # Update axis if needed
        if hasattr(self, "vehicle_axis_y") and hasattr(self.vehicle_axis_y, "setRange"):
            max_count = max(vehicles_by_type.values()) if vehicles_by_type else 1
            self.vehicle_axis_y.setRange(0, max(5, max_count * 1.1))
